#!/usr/bin/env python3
"""
ML Pipeline Testing Suite for PPM Project
...
class MLPipelineTester:
    """Comprehensive ML pipeline testing class"""
    ...
    def test_data_generation(self):
        ...
        try:
            ...
            places_data = fetch_nearby_places(self.test_lat, self.test_lng, radius=2000)
            ...
            if events_data and "results" in events_data:
                ...
                test_result["data_sources"]["predicthq"] = {
                    "status": "warning",
                    "error": "No events returned (may be normal for test period)",
                }
            ...
            if "venues" in self.test_data and self.test_data["venues"]:
                ...
                traffic_data = fetch_foot_traffic(venue_id, venue_type)
            ...
            try:
                weather_data = fetch_current_weather(self.test_lat, self.test_lng)
                if weather_data:
                    ...
                    print(
                        f"✅ Generated weather data: {weather_data.get('weather', [{}])[0].get('main', 'Unknown')}"
                    )
                else:
                    test_result["data_sources"]["weather"] = {
                        "status": "error",
    ...
    def create_mock_venues_dataframe(self):
        ...
        if "venues" in self.test_data:
            ...
            for venue in self.test_data["venues"]:
                venues_data.append(
                    {
                        "venue_id": venue.get("place_id", f"venue_{len(venues_data)}"),
