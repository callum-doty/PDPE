#!/usr/bin/env python3
"""
Simple Event Scraper Interactive Map Generator
Creates an interactive map focused on scraped Kansas City events
Uses the KC Event Scraper with direct Folium integration
"""
...
class SimpleEventMapGenerator:
    """Simple map generator for scraped event data using Folium directly"""
    ...
    def create_event_map(
        ...
        try:
            ...
            # Add CartoDB Positron tiles as an option
            folium.TileLayer(
            ...
            for category, config in self.category_config.items():
                layer = folium.FeatureGroup(
                    name=f"{config['icon']} {category.replace('_', ' ').title()}",
                    show=True,
                )
            ...
            logger.info(f"✅ Event map created: {output_file}")
            return output_file
...
def main():
    ...
    print("🎭 KC Event Scraper Map Generator (Simple)")
    print("=" * 50)
    ...
    try:
        ...
        generator = SimpleEventMapGenerator(use_cached_data=use_cached)
        ...
        # Create map
        map_file = generator.create_event_map(enriched_events, output_path=output_file)
